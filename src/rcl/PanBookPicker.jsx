...
export default function PanBookPicker({
  bookCode,
  setBookCode,
  bookAbbr,
  setBookAbbr,
  ...
  const isProtestantBooksOnlyCheckboxEnabled =
    clientConfig?.["core-contenthandler_text_translation"]
      ...
      ?.fields?.find((field) => field.id === "protestantBooksOnlyCheckbox")
      ?.value !== false;
  ...
  const isProtestantBooksOnlyDefaultChecked =
    clientConfig?.["core-contenthandler_text_translation"]
      ?.find((section) => section.id === "config")
      ?.fields?.find(
        (field) => field.id === "protestantBooksOnlyDefaultChecked",
      )?.value !== false;
              ...
              {(protestantOnly ? bookCodes.slice(0, 66) : bookCodes).map(
                (listItem, n) => (
                  <MenuItem
                    ...
                    dense
                    disabled={
                      bookProject ? bookProject.includes(listItem) : false
                    }
                  >
                    <ListMenuItem
