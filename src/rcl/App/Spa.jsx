...
function Spa({ children }) {
  ...
  const [typography, _setTypography] = useState({
    font_set:
      "Pankosmia-CardoPankosmia-EzraSILPankosmia-PadaukPankosmia-AwamiNastaliqPankosmia-NotoNastaliqUrduPankosmia-GentiumPlus",
    size: "medium",
    direction: "ltr",
  });
  ...
  const [clientConfig, _setClientConfig] = useState({});
  const clientConfigRef = useRef(clientConfig);
  const setClientConfig = (nv) => {
    clientConfigRef.current = nv;
    _setClientConfig(nv);
  };
  ...
  const [clientInterfaces, _setClientInterfaces] = useState({});
  const clientInterfacesRef = useRef(clientInterfaces);
  const setClientInterfaces = (nv) => {
  ...
  const doFetchI18n = async () => {
    ...
    if (i18nResponse.ok) {
      ...
      enqueueSnackbar(`Could not load i18n: ${i18nResponse.error}`, {
        variant: "error",
  ...
  useEffect(() => {
    const doFetchBcv = async () => {
      ...
      if (bcvResponse.ok) {
        ...
        enqueueSnackbar(`Could not load bcv: ${bcvResponse.error}`, {
          variant: "error",
          anchorOrigin: { vertical: "bottom", horizontal: "left" },
          persist: true,
        });
  ...
  const typographyHandler = (ev) => {
    ...
    if (typographyBits.length === 3) {
      ...
      if (
        newTypography.font_set !== typographyRef.current.font_set ||
        newTypography.size !== typographyRef.current.size ||
        newTypography.direction !== typographyRef.current.direction
      ) {
  ...
  const authHandler = (ev) => {
    try {
      ...
      if (["true", "false"].includes(authState)) {
        ...
        if (
          !(authName in newAuth) ||
          !("isActive" in newAuth[authName]) ||
          authBool !== newAuth[authName].isActive
        ) {
  ...
  const currentProjectHandler = (ev) => {
    if (ev.data === null) {
      if (!(currentProjectRef.current === null)) {
        // different types so must be different
      ...
      if (currentProjectBits.length === 3) {
        ...
        if (
          ...
          newCurrentProject.source !== currentProjectRef.current.source ||
          newCurrentProject.organization !==
            currentProjectRef.current.organization ||
          newCurrentProject.project !== currentProjectRef.current.project
        ) {
